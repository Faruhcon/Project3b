...
return-1;},shortestPathAStar:function(grid){this.setupGridData(grid);this.openList=new PriorityQueue();this.closedList=[];this.handleNeighbors(grid,this.gridData[ig.global.START_TILE.y][ig.global.START_TILE.x]);grid[ig.global.START_TILE.y][ig.global.START_TILE.x]=ig.global.START_CLOSED_ID;this.gridData[ig.global.START_TILE.y][ig.global.START_TILE.x].id=ig.global.START_CLOSED_ID;this.gridData[ig.global.START_TILE.y][ig.global.START_TILE.x].closed=true;this.closedList.push(this.gridData[ig.global.START_TILE.y][ig.global.START_TILE.x]);this.pathFound=false;this.count=1;while(!this.openList.isEmpty()&&!this.pathFound&&this.count<this.stopStep){this.count++;var current=this.openList.pop();if(current.id>=ig.global.GOAL_OPEN_ID[0]&&current.id<=ig.global.GOAL_OPEN_ID[7]){this.pathFound=true;}else{this.handleNeighbors(grid,current);grid[current.i.row][current.i.column]+=8;this.gridData[current.i.row][current.i.column].id+=8;}
current.closed=true;this.closedList.push(current);}
this.count++;if(this.pathFound&&this.count<this.stopStep){this.path=[];for(var i=0,parent=this.gridData[ig.global.GOAL_TILE.y][ig.global.GOAL_TILE.x];parent.id!=ig.global.START_CLOSED_ID&&i<1000;i++,parent=parent.parent){this.path[i]=parent;}}},handleNeighbors:function(grid,p){var tid=-1;var start={x:p.i.column,y:p.i.row};if(start.x<ig.global.WIDTH_SCALE-1){tid=this.gridData[start.y][start.x+1].id;this.updateNode(grid,this.gridData[start.y][start.x+1],p,ig.global.ADJACENT_DISTANCE,(tid==ig.global.GOAL_ID||tid==ig.global.GOAL_OPEN_ID[ig.global.ADJACENT_R])?ig.global.GOAL_OPEN_ID[ig.global.ADJACENT_R]:ig.global.ADJACENT_OPEN_ID[ig.global.ADJACENT_R]);}
